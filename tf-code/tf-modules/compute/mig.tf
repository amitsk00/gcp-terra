...
resource "google_compute_instance_template" "lb-mig-tmpl" {
  ...
  disk {
    ...
    source_image = google_compute_image.vm_custom_image.self_link
    auto_delete  = true
    boot         = true
  ...
  # metadata_startup_script = file("gs://proj-2026-main/rhel_startup.sh") 

  depends_on = [google_compute_image.vm_custom_image]
}
